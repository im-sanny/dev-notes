...
When Go Runtime starts, it initializes the following: 1. Goroutine Scheduler 2. Heap Allocator 3. Garbage Collector 4. Logical Processors
...
In short: - Go Runtime creates and manages goroutines. - Goroutines are lightweight, concurrent execution units. - Scheduler runs many goroutines efficiently on few OS threads. - Go Runtime acts as a mini OS inside the process. - Main thread runs the Go Runtime → Go Runtime runs the Main Goroutine.
